...
void app_forcedReport(uint8_t endpoint, uint16_t claster_id, uint16_t attr_id) {
    ...
    if (zb_isDeviceJoinedNwk()) {
        ...
        dstEpInfo.dstAddrMode = 2;
        dstEpInfo.dstEp = 1; 
        dstEpInfo.dstAddr.shortAddr = 0x0000; // Адрес координатора
...
//            printf("MSI report. seqNum: %d\r\n", last_seqNum[i]);
            ret = zcl_report(endpoint+40, &dstEpInfo, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR, last_seqNum[i],
